...
class GetRecordForCategoryUseCase @Inject constructor(
    private val repository: IRecordRepository,
) : BackgroundExecutingUsecase<Long, Flow<List<RecordWithCategoryAndAccount>>>(
) {
    override suspend fun executeInBackground(request: Long): Flow<List<RecordWithCategoryAndAccount>> = repository.fetchRecordForCategory(request)
}